...
**Changelog from v1.0:**
- FR-4 LLM mode now supports both **local** and **cloud** LLM providers via a `LLM_PROVIDER` sub-config
- Privacy invariant preserved: cloud LLM (when used for any auxiliary task) only ever receives **pre-anonymized / surrogate-form data**, never raw PII
- `LLM_PROVIDER` choice applies consistently across entity resolution, missed-PII scan, title generation, metadata extraction, and fuzzy de-anonymization
...
The system uses Microsoft Presidio for NER-based PII detection, generates realistic surrogate values via Faker for reversible entity types, and applies irreversible hard-redaction placeholders for highly sensitive entity types (credit cards, SSNs, etc.). Entity resolution (clustering name variants like "Danny" and "Daniel Walsh" into the same identity) is handled via configurable strategies: algorithmic name-parsing, LLM inference (local or cloud, with the cloud option operating only on pre-anonymized surrogate data), or disabled entirely.
...
- **Configurable**: Entity types, detection thresholds, resolution strategy, LLM provider choice, and scan behaviors are configurable via environment variables
...
Any LLM call that processes user/document content can be configured to use either a local LLM or a cloud LLM via the global `LLM_PROVIDER` setting (see FR-9). The privacy invariant differs by provider:

| Operation | Local LLM behavior | Cloud LLM behavior |
|-----------|--------------------|--------------------|
| **Thread/conversation title generation** | Operates on raw real content | Operates on **anonymized surrogate-form content only**; output is de-anonymized before storage/display |
| **Document metadata extraction** | Operates on raw real content | Operates on **anonymized surrogate-form content only**; output is de-anonymized before storage/display |
| **Entity resolution** | Operates on raw real PERSON entities | Operates on **already-surrogate values** plus the algorithmic candidate mapping; never receives raw PII |
| **Missed PII scan** | Operates on already-anonymized text | Operates on already-anonymized text (same input either way) |
| **Fuzzy de-anonymization (LLM-mode)** | Operates on placeholder-tokenized text | Operates on placeholder-tokenized text (real names hidden as `<<PH_xxxx>>` tokens) |

**Key invariant:** Regardless of `LLM_PROVIDER` value, no raw real PII is ever transmitted to a cloud LLM endpoint. Cloud-mode auxiliary calls are gated by a pre-anonymization step that strips real PII into surrogate or placeholder form before any network egress.
...
| **LLM** | LLM inference for entity clustering. Provider (local or cloud) is selected via `LLM_PROVIDER` sub-config | Local LLM endpoint OR cloud LLM endpoint, depending on `LLM_PROVIDER` |
...
**FR-4.3**: **LLM mode** MUST support two providers selected via the `LLM_PROVIDER` sub-config:

#### FR-4.3.1: Local LLM provider (`LLM_PROVIDER=local`)
...
#### FR-4.3.2: Cloud LLM provider (`LLM_PROVIDER=cloud`)

- The cloud LLM MUST NOT receive raw PERSON entity strings under any circumstance
- Inputs MUST be pre-anonymized before egress: the system first runs algorithmic pre-clustering (FR-4.2) to assign **provisional surrogates** to all candidate names
- The cloud LLM receives only the **provisional surrogate names** and is asked to identify which provisional surrogates likely refer to the same identity (e.g., "Marcus Smith" + "M. Smith" + "Mr. Smith" form one cluster)
- The cloud LLM's output is a clustering decision over surrogate identifiers; it is then mapped back to real entities locally using the provisional surrogate-to-real registry
- The cloud LLM call MUST be wrapped in a pre-flight assertion that scans the outbound payload for any registered real PII value and aborts the call if a leak is detected
- Fall back to the algorithmic clustering result (already computed as input) if the cloud LLM is unavailable or returns invalid output -- this means cloud-mode failures degrade to algorithmic mode rather than to "no clustering"

#### FR-4.3.3: Provider-agnostic requirements (both local and cloud)

- The LLM is never asked to invent new surrogates -- only to cluster existing identifiers
- The LLM response MUST be parsed and validated against the registry; unparseable responses trigger fallback
- The selected provider MUST be logged on every entity-resolution call for audit purposes
...
- **LLM mode**: LLM call to identify fuzzy references. Provider follows the global `LLM_PROVIDER` setting:
  - **Local provider**: receives the placeholder-tokenized text directly (real names hidden as `<<PH_xxxx>>` tokens; see FR-5.4)
  - **Cloud provider**: receives the placeholder-tokenized text. Because real names are already replaced with `<<PH_xxxx>>` tokens before this stage, the cloud LLM never sees raw PII even when fuzzy-matching
...
**FR-8.3**: The missed-PII scan provider follows the global `LLM_PROVIDER` setting (local or cloud). Because the scan operates on **already-anonymized text** (after FR-2 has run), the input contains only surrogates and `[TYPE]` placeholders -- it is therefore safe to send to a cloud LLM. The cloud-mode call MUST still be wrapped in a pre-flight registry-scan assertion (consistent with FR-4.3.2) to confirm no real values leaked through.

**FR-8.4**: Any missed entities found MUST be validated against the configured hard-redact entity set. Invalid entity types are discarded.

**FR-8.5**: If missed entities are found and replaced, the primary NER engine SHOULD re-run on the modified text to recalculate surrogate entity positions (since the text has changed).

### FR-9: LLM Configuration

**FR-9.1**: The system MUST support a global `LLM_PROVIDER` setting (`local` or `cloud`) that selects the auxiliary LLM provider used for: entity resolution, missed-PII scanning, fuzzy de-anonymization, title generation, and metadata extraction.

**FR-9.2**: When `LLM_PROVIDER=local`:
- The system uses the configured local LLM endpoint (`LOCAL_LLM_BASE_URL`, `LOCAL_LLM_MODEL`)
- Auxiliary calls operate on raw real content (titles, metadata) since no network egress to third parties occurs
- No API key is required

**FR-9.3**: When `LLM_PROVIDER=cloud`:
- The system uses the configured cloud LLM endpoint (`CLOUD_LLM_BASE_URL`, `CLOUD_LLM_MODEL`, `CLOUD_LLM_API_KEY`)
- Auxiliary calls MUST operate on **pre-anonymized / surrogate-form content only**
- Every cloud LLM request MUST pass through a pre-flight egress filter that scans the payload against the conversation's real-value registry; any match aborts the request with a logged security error
- Outputs from cloud LLM calls (e.g., generated titles in surrogate form) MUST be de-anonymized locally before storage or display

**FR-9.4**: Both providers MUST use an OpenAI-compatible API (supporting `/v1/chat/completions`). Local: compatible with LM Studio, Ollama, and similar. Cloud: compatible with OpenAI, Anthropic-via-proxy, Together.ai, and similar OpenAI-API-compatible services.

**FR-9.5**: Configuration MUST be available through both environment variables and a settings API/UI.

**FR-9.6**: The system MUST support per-feature provider overrides for advanced deployments (e.g., `ENTITY_RESOLUTION_LLM_PROVIDER=cloud` while keeping `MISSED_SCAN_LLM_PROVIDER=local`). When a per-feature override is unset, the global `LLM_PROVIDER` value applies.

**FR-9.7**: If the selected provider is unavailable (network error, missing config, invalid response), the system MUST gracefully degrade per the rules in NFR-3. Failures MUST be logged but MUST NOT crash the application or leak raw PII as part of the error path.
...
- When `LLM_PROVIDER=cloud` is selected for any auxiliary feature, the pre-flight egress filter (FR-9.3) is a **required defense-in-depth control**, not optional

### NFR-3: Reliability

- If the configured LLM provider is unavailable, auxiliary features MUST gracefully degrade:
  - Entity resolution in LLM mode falls back to algorithmic clustering (cloud provider) or direct 1:1 candidate mapping (local provider)
  - Missed-PII scan is skipped
  - Title generation and metadata extraction fall back to a templated default (e.g., first N tokens of the conversation, with anonymization applied if cloud was the selected provider)
- If both local and cloud providers are configured and one fails, the system MAY attempt the other only if explicitly enabled via `LLM_PROVIDER_FALLBACK_ENABLED` (default: false). Falling back from local to cloud requires the same pre-anonymization guarantees as native cloud mode
...
- Debug-level logging SHOULD capture: entities detected, surrogates assigned, fuzzy matches found, missed PII scan results, UUID filter drops, **LLM provider selected per call**, **pre-flight egress filter results for cloud calls**
...
| `LLM_PROVIDER` | `local` | Global auxiliary LLM provider: `local` or `cloud` |
| `LLM_PROVIDER_FALLBACK_ENABLED` | `false` | If true, allow falling back to the other provider when the configured one fails (cloud fallback still respects pre-anonymization guarantees) |
| `ENTITY_RESOLUTION_LLM_PROVIDER` | *(inherits `LLM_PROVIDER`)* | Per-feature override for entity resolution |
| `MISSED_SCAN_LLM_PROVIDER` | *(inherits `LLM_PROVIDER`)* | Per-feature override for missed-PII scan |
| `TITLE_GEN_LLM_PROVIDER` | *(inherits `LLM_PROVIDER`)* | Per-feature override for title generation |
| `METADATA_LLM_PROVIDER` | *(inherits `LLM_PROVIDER`)* | Per-feature override for metadata extraction |
| `FUZZY_DEANON_LLM_PROVIDER` | *(inherits `LLM_PROVIDER`)* | Per-feature override for fuzzy de-anonymization |
...
| `CLOUD_LLM_BASE_URL` | *(none)* | OpenAI-compatible endpoint for cloud LLM (e.g., `https://api.openai.com/v1`) |
| `CLOUD_LLM_MODEL` | *(none)* | Model name for cloud LLM |
| `CLOUD_LLM_API_KEY` | *(none)* | API key for cloud LLM endpoint (stored in secret manager, not committed) |
...
### 7.6 Why a Pre-Anonymized Cloud LLM Path (Not a Direct Cloud Call)

When `LLM_PROVIDER=cloud` is selected, the system could -- in principle -- send raw PERSON entities to the cloud LLM and get back a clustering. This is rejected because:

- **Violates the system's first-principle invariant**: "No real PII to cloud LLM" is the entire reason this system exists. Allowing one auxiliary feature to bypass it creates a precedent and a leak surface
- **Defense in depth**: Even if a deployer trusts a particular cloud LLM vendor, the pre-anonymization step protects against vendor breach, log retention, training-data leakage, and inference-time prompt logging
- **Symmetric with the main agent path**: The main chat path already pre-anonymizes everything before cloud egress. Auxiliary paths follow the same contract for consistency
- **Graceful degradation is cheaper**: Algorithmic pre-clustering already produces a usable result; the cloud LLM just refines it. Falling back when cloud is unavailable is straightforward

The trade-off is that cloud-mode entity resolution is slightly less powerful than it could be (the cloud LLM works with surrogate names rather than real ones), but the privacy gain is substantial and the quality gap is small in practice.
...
| **Local LLM server** (LM Studio, Ollama, etc.) | OpenAI-compatible local inference | Used when `LLM_PROVIDER=local` for entity resolution, missed-PII scan, title generation, metadata extraction |
| **Cloud LLM endpoint** (OpenAI, Together.ai, or any OpenAI-API-compatible provider) | OpenAI-compatible cloud inference | Used when `LLM_PROVIDER=cloud`. Receives only surrogate-form data. API key managed via secret manager |
...
| Local LLM unavailable | No entity resolution, no missed-PII scan | Graceful degradation to algorithmic clustering or direct mapping; templated fallbacks for title/metadata |
| Cloud LLM unavailable | Auxiliary features degrade | Fall back to algorithmic clustering; missed-PII scan skipped; templated title/metadata fallbacks |
| Cloud LLM payload accidentally contains real PII | Privacy invariant breach | Mandatory pre-flight egress filter (FR-9.3) scans every cloud payload against the conversation registry; logs and aborts on any match |
| Cloud LLM vendor logs / retains prompts | PII (in surrogate form) retained externally | Surrogates are realistic but synthetic; even retained logs contain no real PII. Vendor selection should still favor zero-retention providers |
| Per-feature provider overrides misconfigured | Inconsistent privacy posture across features | Configuration validation at startup; each LLM call logs its resolved provider for audit |
...
- **Differential privacy on cloud-mode payloads**: Add noise/perturbation to cloud LLM inputs even beyond surrogate replacement, for deployments with extreme privacy requirements
- **Vendor-specific cloud LLM adapters**: Beyond OpenAI-compatible API, add native adapters for Anthropic, Google Vertex, AWS Bedrock with their respective auth and retention controls
