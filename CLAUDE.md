...
- The duplicate-024 collision was resolved 2026-05-03: `024_rag_improvements.sql` was renamed to `041_rag_improvements_legacy.sql` (commit `53768e2`). Don't recreate the duplicate. Lesson for future migrations: never reuse a numeric prefix — `supabase_migrations.schema_migrations` uses version as a primary key, and `supabase migration repair` cannot resolve PK collisions.
- **CREATE POLICY needs DROP IF EXISTS guards.** Postgres has no `CREATE POLICY IF NOT EXISTS`, so RLS migrations error with SQLSTATE 42710 if the policy already exists from a partial prior run. Pattern: `DROP POLICY IF EXISTS "name" ON table;` before every `CREATE POLICY`. Migration 039 was patched in `53768e2` to follow this pattern.
