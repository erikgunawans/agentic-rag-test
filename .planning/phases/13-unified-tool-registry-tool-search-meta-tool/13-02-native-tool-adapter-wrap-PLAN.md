...
<task type="auto" tdd="true">
  ...
  <behavior>
    ...
    - Test 2: With `settings.tool_registry_enabled=True` and `_register_natives_with_registry()` invoked, `len(tool_registry._REGISTRY)` equals `len(TOOL_DEFINITIONS)` exactly (no skips, no duplicates).
