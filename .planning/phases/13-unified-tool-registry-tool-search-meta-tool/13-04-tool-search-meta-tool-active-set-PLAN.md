...
<task type="auto" tdd="true">
  ...
  <behavior>
    ...
    - Test 11 (agent filter): registry has search_documents (native), legal_review (skill), tool_search (native), restricted_tool (native). `await tool_search(keyword=".*", regex=None, agent_allowed_tools=["search_documents"])` returns search_documents (allowed) + legal_review (skill bypass) but NOT restricted_tool. Note: tool_search excludes itself per below.
