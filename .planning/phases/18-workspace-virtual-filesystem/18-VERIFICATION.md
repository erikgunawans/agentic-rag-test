...
human_verification:
  ...
  - test: "Open the frontend app, start a chat, trigger a write_file tool call (e.g. ask the agent to 'write a summary to notes/summary.md'), and observe the Workspace Panel sidebar"
    ...
    result: "BLOCKED on 2026-05-03 — Playwright browser drove login + chat input + send message flow successfully; backend rejected POST /threads with 500 because the local OPENAI_API_KEY in backend/.env is stale (openai_service.create_thread() returns 401 from OpenAI). Not a Phase 18 code issue. Re-attempt after refreshing OPENAI_API_KEY locally, or rely on Test 1 + Vitest WorkspacePanel.test.tsx (7/7 green) for component-level coverage."
