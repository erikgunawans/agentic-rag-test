...
*Last updated: 2026-05-04 — **Phase 20 Harness Engine Core + Gatekeeper + Post-Harness + File Upload + Locked PlanPanel COMPLETE** ✅ — 30/35 must-haves verified (HARN×10, MIG-03, GATE×5, POST×5, PANEL×4, UPL×4, SEC×3, OBS×3); 3 critical bugs (CR-01 sentinel trailing-whitespace leak in `gatekeeper.py`, CR-02 `ws` UnboundLocalError in `harness_engine.py`, CR-03 missing `phase_count` in 409 response) recorded as gap-closure items; 3 human UAT items pending (E2E smoke run, CR-01 fix verification, locked PlanPanel visual). 11 plans across 6 waves: 20-01 migration 042 (`harness_runs` table + RLS + partial-unique active-run index + `messages.harness_mode TEXT`), 20-02 `HarnessRunsService` (CRUD + state machine, 12 TDD tests), 20-03 harness engine core (`harnesses/types.py` PhaseType enum + dataclasses, `harness_registry.py`, `harness_engine.py` 9-event SSE suite + dual-cancel, gated barrel `__init__.py`, 20 tests), 20-04 `gatekeeper.py` (20-char sliding-window sentinel, B4 single-registry helper in chat.py, D-02/D-05 routing branches, cancel + active endpoints, 24 tests), 20-05 `post_harness.py` (deterministic truncation D-10, inline SSE streaming, 15 tests), 20-06 file upload endpoint (`workspace_enabled` gate, magic-byte validation, 25MB cap, binary storage, 15 tests), 20-07 smoke-echo 2-phase harness (PROGRAMMATIC + LLM_SINGLE, gated on `harness_smoke_enabled`, 10 tests), 20-08 locked PlanPanel variant (Lock icon + Cancel Dialog, PANEL-02 phase-progression test, 19 i18n keys, 17 Vitest tests), 20-09 HarnessBanner + useChatState SSE slice (6 reducer arms, 3s terminal fade, `/harness/active` rehydration, 409 reject toast, 122 tests pass), 20-10 FileUploadButton + `uploadingFiles` state slice (paperclip button in MessageInput + WelcomeInput, `workspace_enabled` exposed via `/settings/public`, 9 tests), 20-11 cross-cut verification suite (25 SEC/OBS/RLS/B3/B4 integration tests + 14 E2E smoke tests covering SC-1..SC-6). Migration 042 applied to Supabase production (qedhulpfezucnfadlfiz). All features dark-launched (`harness_enabled=False`, `harness_smoke_enabled=False` defaults). Phase 21 (Batched Parallel Sub-Agents + Human-in-the-Loop) next.*
...
*Last updated: 2026-05-05 — **Phase 22 Contract Review Harness + DOCX Deliverable COMPLETE** ✅ — Verifier confirmed 16/16 must-haves (CR-01..08, DOCX-01..08) and all 12 cross-AI REVIEW findings closed (#1 no analyze_document refs, #2 LLM_BATCH_AGENTS canonical merge shape via `_parse_subagent_json_terminal`, #3 filter joins original_text from clauses.json, #4 programmatic executors receive registry → SEC-04 egress filter on internal LLM calls, #5 PyPDF2 in backend runtime, #6 pinned `SandboxService.execute()` API, #7 non-fatal post_execute, #8 deterministic harness_run_id chip correlation via workspace_updated SSE re-emit, #9 e2e test mirrors real HIL pause+resume, #10 search_documents_by_doc_ids overfetch+filter, #11 i18n in ID + EN, #12 tool_service.py frozen-range sha256 `cb63cf3e60d5f6380e2e41f63b1fe9122f58ab4648808b0f2114e452cd45ddb2` unchanged). 12 plans across 7 waves shipped: 22-01 sandbox+backend deps (python-docx 1.1.2 + PyPDF2 3.0.1), 22-02 two new RAG tools adapter-wrapped below tool_service.py:1283 (`list_playbook_documents` + `search_documents_by_doc_ids`), 22-03 `post_execute` hook + `workspace_updated` SSE re-emit in harness_engine, 22-04 workspace-aware gatekeeper system prompt + few-shots (CR-21-08 fix), 22-05 15-phrasing gatekeeper eval set + parametrized pytest + live-LLM script, 22-06 9-phase HarnessDefinition skeleton + CR-01 intake (PROGRAMMATIC) + CR-02 classification (LLM_SINGLE) + `WorkspaceService.read_binary_file`, 22-07 CR-03 LLM_HUMAN_INPUT context-gather + CR-04 LLM_AGENT playbook-load with curated RAG tools, 22-08 CR-05 PROGRAMMATIC clause extraction with registry-routed internal LLM (REVIEW #4) + smoke_echo signature parity, 22-09 CR-06 LLM_BATCH_AGENTS risk analysis + programmatic filter step (REVIEW #2/#3) + CR-07 LLM_BATCH_AGENTS redlines, 22-10 CR-08 LLM_SINGLE summary + post_execute DOCX generation via pinned SandboxService API, 22-11 frontend (`WorkspaceFile.source` widened to include `'harness'` + harness_artifact reducer + DOCX download chip + i18n in ID/EN), 22-12 e2e pytest with two `run_harness_engine` invocations exercising HIL pause+resume on synthetic 3-clause MSA fixture. 166 backend pytest + 6 frontend Vitest passing, frozen-range invariant intact. One cross-plan compat fix during execution (Wave 5's defensive `analyze_document` comments tripped Wave 3's substring guard — rephrased to "no legacy v1.0 tool aliases"). Feature flag `contract_review_enabled` defaults False (D-16 dark-launch); ISSUE-14 deploy-order requires plans 22-06..22-10 all in production before flip. 3 human UAT items pending in `22-HUMAN-UAT.md` (full pytest at runtime, live harness execution end-to-end, gatekeeper workspace-aware trigger). **Milestone v1.3 Agent Harness & Domain-Specific Workflows COMPLETE** — Phases 17–22 all shipped, Contract Review Harness is the first domain implementation. Next: `/gsd-new-milestone` to scope v1.4 OR run pending UAT for Phases 17–22.*
