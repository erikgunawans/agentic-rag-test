...
- [x] 19-04-PLAN.md — task tool registration (adapter-wrap) + chat.py task dispatch with SSE forwarding
- [x] 19-05-PLAN.md — ask_user tool registration + resume-detection branch + chat.py ask_user pause handler
...
- [x] 20-01-PLAN.md — Migration 042 (harness_runs table + RLS + messages.harness_mode column) + BLOCKING supabase db push
- [x] 20-02-PLAN.md — harness_runs_service.py (CRUD + state-machine helpers; mirrors agent_runs_service)
- [x] 20-03-PLAN.md — Harness engine core (types, registry, engine, auto-import barrel) + HARNESS_ENABLED/HARNESS_SMOKE_ENABLED flags
- [x] 20-04-PLAN.md — Gatekeeper service + chat.py routing branches (D-02 reject, D-05 gatekeeper, cancel + active endpoints)
- [x] 20-05-PLAN.md — Post-harness summary service + inline SSE handoff in _gatekeeper_stream_wrapper
- [x] 20-06-PLAN.md — File upload backend (POST /threads/{id}/files/upload + register_uploaded_file) — magic-byte + 25 MB cap
- [x] 20-07-PLAN.md — Smoke harness (smoke_echo.py — 2-phase programmatic+llm_single, gated on HARNESS_SMOKE_ENABLED)
- [x] 20-08-PLAN.md — Locked Plan Panel variant (lock icon + tooltip + Cancel Dialog) + i18n keys
- [x] 20-09-PLAN.md — useChatState harnessRun slice + HarnessBanner component + ChatPage slot
- [x] 20-10-PLAN.md — FileUploadButton (paperclip + multipart upload + AbortController) + MessageInput/WelcomeInput slots
- [x] 20-11-PLAN.md — Cross-cuts verification (SEC-02..04 + OBS-01..03 + RLS isolation + OFF-mode byte-identical) + E2E smoke test
**UI hint**: yes
...
- [x] 22-05-PLAN.md — Gatekeeper trigger eval set (15 phrasings) + CI test (D-22-04)
- [x] 22-06-PLAN.md — contract_review.py skeleton + CR-01 intake + CR-02 classification (flag-gated)
...
| 22. Contract Review Harness + DOCX Deliverable | 16/16 | Complete   | 2026-05-06 |
