...
**Plans:** 10 plans
- [ ] 19-01-PLAN.md — Migration 040 (agent_runs + messages.parent_task_id) + RLS + BLOCKING supabase db push
- [ ] 19-02-PLAN.md — agent_runs_service (CRUD + state-machine + resume detection)
- [ ] 19-03-PLAN.md — sub_agent_loop module (async generator, summary fallback, structured-error wrapper) + SUB_AGENT_ENABLED config
- [ ] 19-04-PLAN.md — task tool registration (adapter-wrap) + chat.py task dispatch with SSE forwarding
- [ ] 19-05-PLAN.md — ask_user tool registration + resume-detection branch + chat.py ask_user pause handler
- [ ] 19-06-PLAN.md — agent_status SSE emission (4 sites) + agent_runs lifecycle + append-only error contract
- [ ] 19-07-PLAN.md — Frontend: useChatState slices + AgentStatusChip + TaskPanel + MessageView question-bubble + i18n
- [ ] 19-08-PLAN.md — Replace Phase 17 stubs in deep_mode_prompt.py with real sub-agent / ask-user / error-recovery guidance
...
| 19. Sub-Agent Delegation + Ask User + Status & Recovery | 0/10 | Planned    | — |
